...
function guessPlatform(row, sourceFile = "") {
  ...
  if (/douyin|抖音|iesdouyin/.test(joined) || hasToken(joined, "dy")) return "douyin";
  if (/bilibili|b23\.tv|b站|哔哩/.test(joined) || hasToken(joined, "bili")) return "bilibili";
  if (/xiaohongshu|xhslink|小红书/.test(joined) || hasToken(joined, "xhs")) return "xhs";
  if (/kuaishou|gifshow|快手/.test(joined) || hasToken(joined, "ks")) return "kuaishou";
  if (/weibo|微博/.test(joined) || hasToken(joined, "wb")) return "weibo";
