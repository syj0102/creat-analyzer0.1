...
function guessPlatform(row, sourceFile = "") {
  ...
  if (/douyin|抖音|iesdouyin/.test(joined)) return "douyin";
  if (/bilibili|b23\.tv|b站|哔哩/.test(joined)) return "bilibili";
  if (/xiaohongshu|xhslink|小红书/.test(joined)) return "xhs";
  if (/kuaishou|gifshow|快手/.test(joined)) return "kuaishou";
  if (/weibo|微博/.test(joined)) return "weibo";
